...
#         if result[1] < 500:
#             Confidence = int(100 * (1 - (result[1]) / 300))
#             # Use f-strings instead of + to format the strings
...
#         if Confidence > 65:
#             # Get the user name from the user_names list using the label
...
while True:
    ...
    try:
        ...
        if Confidence > 65:
            ...
            cv2.imshow("Face Cropper", image)
            # Load the excel file and the sheet as a pandas dataframe
            df = pd.read_excel(excel_path, sheet_name=sheet_name, index_col="Name")
            # Check if the user has been marked as present before
            if df.loc[user_name, "Attendance"] == "Absent":
                # Mark the user as present and update the excel file
                df.loc[user_name, "Attendance"] = "Present"
                # Get the current time and format it as HH:MM:SS
                current_time = datetime.datetime.now().strftime("%H:%M:%S")
                # Store the current time in the excel file
                df.loc[user_name, "Time"] = current_time
                df.to_excel(excel_path, sheet_name=sheet_name)
                # Speak the user name and the time of attendance
                speak(f"{user_name} is present at {current_time}")
            else:
                # Speak that the user has already been marked as present
                speak(f"{user_name} has already been marked as present")
    ...
    # Add an except block to handle the possible exceptions
    except Exception as e:
        # Print the exception message
        print(e)
        # Speak that an error has occurred
        speak("An error has occurred")
        # Show the image without any changes
        cv2.imshow("Face Cropper", image)

    # Add a finally block to execute the code regardless of the exception
    finally:
        # Check if the user presses the ESC key
        if cv2.waitKey(1) == 27:
            # Break the loop and release the resources
            break
